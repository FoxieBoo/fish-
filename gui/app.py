...
CUSTOM_TUNABLE_PARAMS = [
    ("摇头间隔(s)",   "SHAKE_HEAD_GAP",   "float", "两段摇头之间的等待时间"),
    ("复位重发(次)",  "SHAKE_HEAD_RESET_REPEAT", "int", "摇头前后发送Left/Right=0的次数"),
    ("复位间隔(s)",   "SHAKE_HEAD_RESET_INTERVAL", "float", "复位重发之间的间隔时间"),
    ("快锁跳变(px)",  "FAST_LOCK_JUMP_PX", "int", "鱼单帧跳动超过此值时减少平滑滞后"),
    ("快锁速度(px/s)", "FAST_LOCK_SPEED_PX_S", "float", "鱼速度超过此值触发快速锁中"),
    ("快锁前瞻(ms)",  "FAST_LOCK_LOOKAHEAD_S", "ms", "快速锁中使用的预测提前量"),
    ("快锁触发阈(%)", "FAST_LOCK_TRIGGER_ERR", "pct", "偏离中心超过此比例触发快锁"),
    ("快锁增益",      "FAST_LOCK_BOOST_GAIN", "float", "快速锁中附加按压增益"),
    ("快锁增压(ms)",  "FAST_LOCK_BOOST_MAX_S", "ms", "快速锁中最多额外按压时长"),
    ("快锁下落阈(%)", "FAST_LOCK_DROP_ERR", "pct", "鱼明显在条下方时快速松开阈值"),
]

TUNABLE_PARAMS = SOURCE_TUNABLE_PARAMS + CUSTOM_TUNABLE_PARAMS
...
class FishingApp:
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(frm_toggles, text="快速锁中",
                        variable=self.var_fast_lock,
                        command=self._on_fast_lock_toggle).pack(
                            side="left", padx=5
                        )
    ...
    def _build_params_panel(self, pad):
        ...
        row = 0
        row = self._render_param_group(frm, SOURCE_TUNABLE_PARAMS, row, "源项目参数")
        ttk.Separator(frm, orient="horizontal").grid(
            row=row, column=0, columnspan=6, sticky="ew", padx=6, pady=(4, 2)
        )
        row += 1
        row = self._render_param_group(frm, CUSTOM_TUNABLE_PARAMS, row, "自定义扩展参数")

        # 按钮行
        btn_frame = ttk.Frame(frm)
        btn_frame.grid(row=row, column=0, columnspan=6,
    ...
    def _render_param_group(self, parent, params, start_row: int, title: str) -> int:
        """在参数面板中渲染一个分组，返回下一可用行号。"""
        cols_per_row = 2
        gpad = {"padx": 4, "pady": 2}
        ...
        for i, (label, attr, vtype, tip) in enumerate(params):
        ...
        rows_used = (len(params) + cols_per_row - 1) // cols_per_row
    ...
    def _apply_fast_lock_preset(self, preset_key: str):
        ...
        self._save_settings()
        name = "高难快锁" if preset_key == "aggressive" else "平衡快锁"
        self._log_msg(f"[参数] 已应用预设: {name}")
    ...
    def _on_fast_lock_toggle(self):
        ...
        self._save_settings()
        state = "开启" if config.FAST_LOCK_ENABLED else "关闭"
        self._log_msg(f"[控制] 快速锁中: {state}")
