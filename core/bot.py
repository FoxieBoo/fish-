...
class FishingBot:
    ...
    def __init__(self):
        ...
        self._fish_prev_cy  = None       # 上一帧鱼中心 Y (计算鱼速度)
        self._fish_prev_time = None      # 上一帧鱼时间戳
        ...
        if fish is not None and bar is not None:
            ...
            if hold >= MIN_HOLD + 0.001:
                ...
                log.info(
                    ...
                    f"{' ⚡' if fast_trigger else ''} → 按 {hold*1000:.0f}ms"
                )
                return True
            else:
                self.input.mouse_up()
